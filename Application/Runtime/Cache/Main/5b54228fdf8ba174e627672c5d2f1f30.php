...
<div class="main">
    <form method="post" action="<?php echo U('dosave');?>">
        ...
        <label>类型</label>
        <select name="type">
            <option value="1" <?php if($area["type"] == 1): ?>selected<?php endif; ?> >微信</option>
            <option value="2"<?php if($area["type"] == 2): ?>selected<?php endif; ?> >QQ</option>
        </select>
        
        <label>区号</label>
        <input type="text" name="area_no" value="<?php echo ($area["area_no"]); ?>" />
		
		<label>区名</label>
        <input type="text" name="area_name" value="<?php echo ($area["area_name"]); ?>" />
        <button>提交</button>
    </form>
</div>
</section>
